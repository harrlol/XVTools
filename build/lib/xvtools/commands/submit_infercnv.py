...
def infercnv_aml(
    ...
    _run(["azcopy", "sync", str(data), g_dst, "--recursive", "--delete-destination=false"])
    rprint("[cyan]Uploaded input to Azure blob[/]")

    # Submit AMLT job (requires amlt configured)
    _run(["amlt", "run", str(resolved), "--replace", job_name, "-d", job_name])
    rprint(f"[green]Submitted[/] {job_name}")

    while True:
        s = _run(["amlt", "status", job_name])
        if any(x in s for x in ("queued","scheduling","preparing","starting","running")):
            typer.echo(s.strip().splitlines()[-1])
            typer.sleep(120)
            continue
        if any(x in s for x in ("failed","canceled")):
            rprint("[red]Job failed or canceled[/]")
            return
        if "completed" in s or "pass" in s:
